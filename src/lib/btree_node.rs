	...
	V: Sized + Default,
{
        ...
        Q: Ord,
    {
        match self.find_key_index(key) {
           ...
           IndexResult::GoDown(index) => return {
               if self.children[index].is_some() {
                   ...
                   match child.search(key) {
                       SearchResult::Found(index) => return {
                           SearchResult::GoDown(unsafe { &*(&child.values[index] as *const V as *const V) }) },
                       SearchResult::GoDown(value_ref) => return SearchResult::GoDown( unsafe { &*(value_ref as *const V as *const V) }),
    ...
    fn check_child_underflow(&mut self, child_idx: usize)
    {
        ...
        if (child.length as usize) < B - 1 { // Child node underflow
    ...
    fn merge(&mut self, merge_to_idx: usize, mut left_child: RefMut<Node<K, V>>, mut right_child: RefMut<Node<K, V>>)
    {
    ...
    fn borrow_from_left(&mut self, child_idx: usize, mut child: RefMut<Node<K, V>>, mut left_sibling: RefMut<Node<K, V>>)
    {
